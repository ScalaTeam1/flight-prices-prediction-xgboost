...
case class FightPricePredictor(
    modelId: String,
    model: XGBoostRegressionModel,
    preprocessor: PipelineModel
) extends Predictor[Flight] {
...
object FightPricePredictor {
  ...
  def apply(
      modelId: String,
      model: XGBoostRegressionModel,
      preprocessorModel: PipelineModel
  ) = new FightPricePredictor(modelId, model, preprocessorModel)
  def loadModel(path: String) = XGBoostRegressionModel.load(path)
  def loadPreprocessModel(path: String): PipelineModel =
