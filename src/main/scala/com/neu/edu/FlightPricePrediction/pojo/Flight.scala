...
object Flight extends TableParserHelper[Flight]() {
  ...
  def camelCaseColumnNameMapper(w: String): String =
    w.replaceAll("([A-Z0-9])", "_$1")

  implicit val FlightsColumnHelper: ColumnHelper[Flight] =
    columnHelper(camelCaseColumnNameMapper _, "classType" -> "class")
  ...
  implicit object FlightTableParser extends StringTableParser[Table[Flight]] {
    protected def builder(
        rows: Iterable[Flight],
        header: Header
    ): Table[Flight] = HeadedTable(rows, header)
